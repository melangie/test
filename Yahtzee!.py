...
fps = 60
font = pygame.font.Font('font/Louis George Cafe.ttf', 19)
font1 = pygame.font.Font('font/Louis George Cafe.ttf', 25)
font2 = pygame.font.Font('font/Louis George Cafe.ttf', 15)
roll = False
numbers = [7, 8, 9, 10, 11]
...
score = 0
restart = ''
background1 = pygame.image.load('backgrounds/blue.png')
background2 = pygame.image.load('backgrounds/green.png')
background3 = pygame.image.load('backgrounds/purple.png')
background4 = pygame.image.load('backgrounds/red.png')
background5 = pygame.image.load('backgrounds/yellow.png')
running = True
...
class Choice:
    ...
    def draw(self):
        pygame.draw.line(screen, (0, 0, 0), (self.x_pos, self.y_pos + 31), (self.x_pos + 225, self.y_pos + 31), 1)
        name_text = ''
        if not self.done:
            if self.possible:
                name_text = font.render(self.text, True, (79, 150, 78))
            elif not self.possible:
                name_text = font.render(self.text, True, (219, 18, 71))
        else:
            name_text = font.render(self.text, True, (0, 0, 0))
        if self.selected:
            pygame.draw.rect(screen, (0, 0, 0), [self.x_pos, self.y_pos + 2, 155, 30])
        screen.blit(name_text, (self.x_pos + 5, self.y_pos + 6))
        score_text = font.render(str(self.score), True, (6, 72, 148))
        screen.blit(score_text, (self.x_pos + 165, self.y_pos + 6))
...
class Dice:
    ...
    def draw(self):
        self.die = pygame.draw.rect(screen, (255, 255, 255), [self.x_pos, self.y_pos, 100, 100], 0, 10)
        if self.number == 1:
            pygame.draw.circle(screen, (150, 0, 0), (self.x_pos + 50, self.y_pos + 50), 10)
        if self.number == 2:
        ...
        if self.number == 6:
            ...
            pygame.draw.circle(screen, (0, 0, 0), (self.x_pos + 80, self.y_pos + 20), 10)
        if self.active:
            pygame.draw.rect(screen, (223, 18, 71), [self.x_pos, self.y_pos, 100, 100], 4, 10)
...
def draw_stuff():
    global game_over
    global game_score
    roll_text = font.render('Click to roll', True, (255, 255, 255))
    screen.blit(roll_text, (105, 164))
    accept_text = font.render('Accept turn', True, (255, 255, 255))
    screen.blit(accept_text, (390, 164))
    turns_text = font.render('Rolls left this turn: ' + str(rolls_left), True, (0, 0, 0))
    screen.blit(turns_text, (38, 10))
    but_text = font.render('Click a die to keep it or release it', True, (0, 0, 0))
    screen.blit(but_text, (287, 10))
    pygame.draw.rect(screen, (255, 255, 255), [0, 200, 225, height - 200])
    pygame.draw.line(screen, (0, 0, 0), (0, 40), (width, 40), 2)
    pygame.draw.line(screen, (0, 0, 0), (0, 200), (width, 200), 2)
    pygame.draw.line(screen, (0, 0, 0), (600, 0), (600, 200), 2)
    pygame.draw.line(screen, (0, 0, 0), (250, 0), (250, 40), 2)
    pygame.draw.line(screen, (0, 0, 0), (155, 200), (155, height), 2)
    pygame.draw.line(screen, (0, 0, 0), (225, 200), (225, height), 3)
    if game_over:
        over_text = font.render('Game Over! Click here to play again', True, (255, 255, 255))
        screen.blit(over_text, (262, 380))
    score_text = font1.render('Score: ' + str(game_score), True, (0, 0, 0))
    screen.blit(score_text, (367, 300))
    highscore_text = font1.render('High scores:', True, (0, 0, 0))
    screen.blit(highscore_text, (350, 450))
    background_info = font2.render('Choose your lucky color by pressing 1-5', True, (0, 0, 0))
    screen.blit(background_info, (280, 760))
...
def check_possibilities(pos_list, nums):
    ...
    for index in range(1, 7):
        if nums.count(index) > max_count:
            max_count = nums.count(index)
    if max_count < 3:
        pos_list[6] = False
        pos_list[7] = False
        pos_list[8] = False
...
def check_totals(totals_list, scores_list, my_bonus):
    ...
    totals_list[4] = scores_list[6] + scores_list[7] + scores_list[8] + \
                     scores_list[9] + scores_list[10] + scores_list[11] + scores_list[12]
    ...
    if game_over:
        new_score = totals_list[6]
        score_set = set(highscores)
        if new_score not in score_set:
            highscores.append(new_score)
            score_set.add(new_score)
            if len(highscores) > 5:
                min_score = min(highscores)
                highscores.remove(min_score)
                score_set.remove(min_score)
    ...
    for j, score in enumerate(highscores):
        text1 = font.render(f'{j + 1}. {score}', True, (0, 0, 0))
        screen.blit(text1, (394, 495 + j * 40))
...
while running:
    ...
    if game_over:
        restart = pygame.draw.rect(screen, (0, 0, 0), [257, 375, 320, 30])
    ...
    three_kind = Choice(0, 470, '3 of a kind', choice[6], possible[6], done[6], scores[6])
    four_kind = Choice(0, 500, '4 of a kind', choice[7], possible[7], done[7], scores[7])
    full_house = Choice(00, 530, 'Full house', choice[8], possible[8], done[8], scores[8])
    small_straight = Choice(0, 560, 'Small straight', choice[9], possible[9], done[9], scores[9])
    large_straight = Choice(0, 590, 'Large straight', choice[10], possible[10], done[10], scores[10])
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_1:
                ...
                background_image = background3
            elif event.key == pygame.K_4:
                background_image = background4
            elif event.key == pygame.K_5:
                background_image = background5
    ...
    for i in range(len(possible)):
        if choice[i] and not possible[i]:
            ...
            screen.blit(click_text, (240, 215))
        if choice[i]:
